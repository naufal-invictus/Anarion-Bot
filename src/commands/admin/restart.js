module.exports = {
    ...
    execute: async (sock, msg, args, userRole) => {
        ...
        if (userRole !== 'owner' && userRole !== 'developer') {
            return sock.sendMessage(msg.key.remoteJid, { text: '⛔ Perintah ini hanya untuk Owner atau Developer.' }, { quoted: msg });
        }

        try {
            await sock.sendMessage(msg.key.remoteJid, { text: '✅ Bot akan di-restart...' }, { quoted: msg });
            // Menggunakan process.exit(1) agar PM2 otomatis menghidupkan kembali bot
            process.exit(1);
        } catch (error) {
            console.error('Error pada perintah !restart:', error);
            await sock.sendMessage(msg.key.remoteJid, { text: 'Gagal me-restart bot.' }, { quoted: msg });
        }
    },
};